//Timeseries lag-1 model 

// data is prepared with rewEff_prepare_retest [analysis/functions/rew_eff_functions.R]
// model is fit with rewEff_fit_retestCMDstan [analysis/functions/rew_eff_functions.R]


// hierarchical item response theory model to get latent state motivation 

// here there are two state effects on rewSens: state[t-1] & state[+1]; which we compare directly 
// state effects on effort sens are removed due to not being meaningful in prior original model 
// everything else is the same as rewEff-linear-bernoulli-multisess-joint-IRT-1param
...
data {
  ...
  array[nPpts, nTaskTimes, nTrials_max] int<lower=0, upper=99> choice01;
...
parameters {
  ...
  vector<lower=0>[nTaskTimes] sigma_rewSens;

  

  vector<lower=0>[nTaskTimes] sigma_effSens;

  

  vector<lower=0>[nStateTimes] sigma_theta;
  ...
  matrix[nTaskTimes, nPpts] rewSens_raw;

  

  matrix[nTaskTimes, nPpts] effSens_raw;
  ...
  real beta_rew_prev; // t-1 

  

  real beta_rew_next; // t+2 
  
  // no effSens state effect in this model
  ...
  for (p in 1 : nPpts) {
    ...
    for (t in 1 : nTaskTimes) {
      ...
      if (is_missing[p, t, 1] == 0) {
        ...
        // trait apathy score acts as intercept with magnitude and direction @ group level determined by trait betas

        // allow theta to influnce task parameters and quantify with beta weight 
        
        // -1 and +1 states compete for variance (ss indicates the task index within the state array)
        

        rewSens[p, t] = beta_trait_rew * apathyCFA[p] + rewSens_tilde[t, p]

                        + (beta_rew_prev * thetaZ[p, ss[p, t] - 1])

                        + (beta_rew_next * thetaZ[p, ss[p, t] + 1]);
        ...
        effSens[p, t] = beta_trait_eff * apathyCFA[p] + effSens_tilde[t, p];
                ...
                // if missing, game parameter is assigned trait-effect*trait-score only
...
model {
  ...
  beta_rew_prev ~ normal(0.19, 0.08); // prior = beta values found for the current state

  

  beta_rew_next ~ normal(0.19, 0.08); // prior = beta values found for the current state
