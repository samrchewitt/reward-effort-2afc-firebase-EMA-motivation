  ...
  for (p in 1 : nPpts) {
    ...
    for (t in 1 : nTaskTimes) {
      ...
      if (is_missing[p, t, 1] == 0) {
        ...
        // t-1 and t-2 state compete for variance 
        

        rewSens[p, t] = beta_trait_rew * apathyCFA[p] + rewSens_tilde[t, p]

                        + (beta_rew_prev1 * thetaZ[p, ss[p, t] - 1])

                        + (beta_rew_prev2 * thetaZ[p, ss[p, t] - 2]);
...
model {
  ...
  beta_rew_prev1 ~ normal(0.19, 0.08); // prior = beta values found for the current state

  

  beta_rew_prev2 ~ normal(0.19, 0.08); // prior = beta values found for the current state
