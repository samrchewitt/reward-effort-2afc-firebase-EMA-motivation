...
// subjective value transformed directly to choice with bernoulli transform 
...
parameters {
  ...
  // // group means at each timepoint for state motivation
  
  vector[nStateTimes] mu_theta;
  
  // group sd at each timepoint for reward sens, effort sens and state theta
  ...
  // IRT-specific item difficulty parameters
...
transformed parameters {
  ...
  // (may be a nicer way to do this without loops)
  ...
  for (p in 1 : nPpts) {
    ...
    // dissociate state from trait with z-score [thetaZ] (within-person scaling)
  ...
  // 1 parameter IRT is slightly confusing because delta is also a parameter but at the item level (not subject)
...
generated quantities {
  array[N, nStateTimes] real loglik_y; // LL for state endorsements
  ...
  for (p in 1 : nPpts) {
    for (t in 1 : nTaskTimes) {
      ...
      if (is_missing[p, t, 1] == 0) {
        // post_pred_t = model replicated choices
